...
output_dir='../exp/ltuas_ft_meld2/'
mkdir -p $output_dir
cp "$0" ${output_dir}/$(date +"%Y-%m-%d-%H-%M-%S").sh

python ../finetune_low_resource.py \
    --base_model '/root/ltu-main/ltu-main/pretrained_mdls/ltuas_long_noqa_a6.bin' \
    --data_path '/root/ltu-main/ltu-main/openasqa/data/meld_data_reformatted.json'\
