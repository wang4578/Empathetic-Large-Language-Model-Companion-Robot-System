...
class DataCollatorForSeq2Seq:
    ...
    def gen_filename(self, filename):
        ...
        # filename = '_'.join(filename.split('/')[-3:-1]) + '_' + filename.split('/')[-1][:-len(ext) - 1]
        # # TODO: change it to your own whisper feature path
        # #filename = '../../../openasqa/data/sample_audio_feat/whisper_feat_titan/' + filename + '.npz'
        # filename = '/root/ltu-main/ltu-main/whisper_large-v1/' + filename + '.npz'
        
        # 新逻辑
        ext = filename.split('.')[-1]  # 获取扩展名，例如 'flac'
        # 从路径中提取数据集类型（dev/test）
        if '/dev/' in filename:
            dataset_type = 'dev'
        elif '/test/' in filename:
            dataset_type = 'test'
        else:
            dataset_type = 'train_splits_wav'
            
        # 获取文件名（不含扩展名）
        base_filename = filename.split('/')[-1][:-len(ext)-1]  # 例如 'dia0_utt0'
        
        # 构建新的文件名
        filename = f'/root/ltu-main/ltu-main/whisper_large-v1/data_MELD.Raw_{dataset_type}_{base_filename}.npz'
        return filename
