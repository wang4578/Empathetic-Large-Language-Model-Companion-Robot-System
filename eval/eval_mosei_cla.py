...
directory = '/root/data/MELD.Raw/dev_test/'
...
eval_file_list = [
    '/root/ltu-main/ltu-main/eval_data/eval_data3'
]
eval_file_list = [x + '.json' for x in eval_file_list]
# print(eval_file_list)
...
local_model_path = "/root/ltu-main/ltu-main/bert_large_uncased" 
bert_mdl_size = 'bert-large-uncased'
# bert_tokenizer = AutoTokenizer.from_pretrained(bert_mdl_size, model_max_length=512)
bert_tokenizer = AutoTokenizer.from_pretrained(local_model_path, model_max_length=512)
...
for eval_file in eval_file_list:
    ...
    label_list = np.loadtxt('/root/ltu-main/ltu-main/eval_data/meld_emotion_eval.csv', delimiter=',', dtype=str, skiprows=1)
    ...
    if os.path.exists('/root/ltu-main/ltu-main/eval_data/{:s}_{:s}_{:s}.json'.format(dataset, llm_task, text_embed_setting)) == True:
        with open('/root/ltu-main/ltu-main/eval_data/{:s}_{:s}_{:s}.json'.format(dataset, llm_task, text_embed_setting), 'r') as f:
            embed_cache = f.read()
    ...
    save_fold = "/root/ltu-main/ltu-main/eval_data/{:s}_{:s}_{:s}_cla_report".format(
        '.'.join(eval_file.split('/')[-1].split('.')[:-1]), llm_task, text_embed_setting)
    ...
    save_cache_path = '/root/ltu-main/ltu-main/eval_data/{:s}_{:s}_{:s}.json'.format(dataset, llm_task, text_embed_setting)

    with open(save_cache_path, 'w') as f:
        f.write(embed_cache)
    y_true = np.argmax(all_truth, axis=1)
    y_pred = np.argmax(all_pred, axis=1)
    sk_acc = accuracy_score(y_true, y_pred)
    f1 = f1_score(y_true, y_pred, average='weighted')
    # sk_acc = accuracy_score(all_truth, all_pred)
    # f1 = f1_score(all_truth, all_pred, average='weighted')
    print('{:s} accuracy: {:.4f}, F1: {:.4f}'.format(dataset, sk_acc, f1))
    ...
    all_res.append([eval_file, acc, f1])
    ...
    np.savetxt('/root/ltu-main/ltu-main/eval_data/summary_iemocap_emotion_{:s}.csv'.format(time_string), all_res, delimiter=',', fmt='%s')
